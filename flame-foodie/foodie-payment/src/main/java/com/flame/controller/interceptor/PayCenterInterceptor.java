...
import com.flame.utils.DateUtil;
...
public class PayCenterInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, 
		...
		String flameUserId = request.getHeader("flameUserId");
		String password = request.getHeader("password");

		if (StringUtils.isNotBlank(flameUserId) && StringUtils.isNotBlank(password)) {

			// 请求数据库查询用户是否存在
			Users  user = userService.queryUserInfo(flameUserId, password);
			if (user == null) {
				returnErrorResponse(response, new FlameJSONResult().errorTokenMsg("用户id或密码不正确！"));
				return false;
			}
			...
			if (days < 0) {
				returnErrorResponse(response, new FlameJSONResult().errorTokenMsg("该账户授权访问日期已失效！"));
				return false;
			}
			...
			returnErrorResponse(response, new FlameJSONResult().errorTokenMsg("请在header中携带支付中心所需的用户id以及密码！"));
			return false;
		}

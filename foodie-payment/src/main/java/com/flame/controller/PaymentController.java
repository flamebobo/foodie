...
import com.flame.enums.PaymentStatus;
import com.flame.pojo.Orders;
import com.flame.pojo.bo.MerchantOrdersBO;
import com.flame.pojo.vo.PaymentInfoVO;
import com.flame.resource.AliPayResource;
import com.flame.resource.WXPayResource;
import com.flame.service.PaymentOrderService;
import com.flame.utils.CurrencyUtils;
import com.flame.utils.FlameJSONResult;
import com.flame.utils.RedisOperator;
import com.flame.wx.entity.PreOrderResult;
import com.flame.wx.service.WxOrderService;
...
public class PaymentController {
	...
	public FlameJSONResult createMerchantOrder(@RequestBody MerchantOrdersBO merchantOrdersBO, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (StringUtils.isBlank(merchantOrderId)) {
			return FlameJSONResult.errorMsg("参数[orderId]不能为空");
		}
		if (StringUtils.isBlank(merchantUserId)) {
			return FlameJSONResult.errorMsg("参数[userId]不能为空");
		}
		if (amount == null || amount < 1) {
			return FlameJSONResult.errorMsg("参数[realPayAmount]不能为空并且不能小于1");
		}
		if (payMethod == null) {
			return FlameJSONResult.errorMsg("参数[payMethod]不能为空并且不能小于1");
		}
		if (payMethod != PayMethod.WEIXIN.type && payMethod != PayMethod.ALIPAY.type) {
			return FlameJSONResult.errorMsg("参数[payMethod]目前只支持微信支付或支付宝支付");
		}
		if (StringUtils.isBlank(returnUrl)) {
			return FlameJSONResult.errorMsg("参数[returnUrl]不能为空");
		}
		...
		try {
			...
			e.printStackTrace();
			FlameJSONResult.errorException(e.getMessage());
		}

		if (isSuccess) {
			return FlameJSONResult.ok("商户订单创建成功！");
		} else {
			return FlameJSONResult.errorMsg("商户订单创建失败，请重试...");
		}
	}
	...
	public FlameJSONResult getPaymentCenterOrderInfo(String merchantOrderId, String merchantUserId) {

		if (StringUtils.isBlank(merchantOrderId) || StringUtils.isBlank(merchantUserId)) {
			return FlameJSONResult.errorMsg("查询参数不能为空！");
		}

		Orders orderInfo = paymentOrderService.queryOrderInfo(merchantUserId, merchantOrderId);

		return FlameJSONResult.ok(orderInfo);
	}
...
//	@GetMapping(value="/getWXPayQRCode")
	...
	public FlameJSONResult getWXPayQRCode(String merchantOrderId, String merchantUserId) throws Exception{
...
//				String total_fee = "1";	// 测试用 1分钱
			...
			return FlameJSONResult.ok(paymentInfoVO);
		} else {
			return FlameJSONResult.errorMsg("该订单不存在，或已经支付");
		}
	}
	...
	public FlameJSONResult goAlipay(String merchantOrderId, String merchantUserId) throws Exception{
...
//    	String total_amount = "0.01";	// 测试用 1分钱
		...
		return FlameJSONResult.ok(alipayForm);
	}
